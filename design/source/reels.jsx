// Reels: 3 independent slot reels (DD numeric, MM word, YYYY numeric).
// Each cell is exactly 60px tall; the centre row is the readout.
...
function Reel({ items, targetIndex, kind, width, spinning, onLanded, delay }){
  ...
  React.useEffect(() => { onLandedRef.current = onLanded; }, [onLanded]);
...
function DateReels({ deathDate, spinning, onAllLanded }){
  const days = React.useMemo(() => Array.from({length: 31}, (_, i) => pad2(i + 1)), []);
  const months = MONTHS;
  ...
  const landedRef = React.useRef({ d: false, m: false, y: false });
  ...
  const markLanded = React.useCallback((key) => {
  ...
  const onD = React.useCallback(() => markLanded('d'), [markLanded]);
  const onM = React.useCallback(() => markLanded('m'), [markLanded]);
  const onY = React.useCallback(() => markLanded('y'), [markLanded]);
